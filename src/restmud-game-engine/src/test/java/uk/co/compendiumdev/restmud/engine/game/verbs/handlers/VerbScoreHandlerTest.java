...
public class VerbScoreHandlerTest {
    ...
    @Test
    public void canGetUpToDateScore(){

        // receive the message
        VerbScoreHandler score = new VerbScoreHandler().setGame(game);
        LastAction action = score.doVerb(player, "");
        Assert.assertTrue(action.isSuccess());
        Assert.assertEquals(1, action.getUserDetailsList().size());
        Assert.assertEquals(0, action.getUserDetailsList().get(0).score.intValue());

        player.setScore(22);

        action = score.doVerb(player, "");
        Assert.assertTrue(action.isSuccess());
        Assert.assertEquals(1, action.getUserDetailsList().size());
        Assert.assertEquals(22, action.getUserDetailsList().get(0).score.intValue());
    ...
    public void verbHandlerConditions(){
        ...
        VerbScoreHandler handler = new VerbScoreHandler().setGame(game).usingCurrentVerb("score");
        Assert.assertFalse("Score takes no time",
                handler.actionUpdatesTimeStamp());
        Assert.assertFalse("Score includes no messages",
                handler.shouldAddGameMessages());
        Assert.assertFalse(
                "no need to look after score",
