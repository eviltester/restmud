...
import uk.co.compendiumdev.restmud.engine.game.scripting.FailGateScriptingTest;
...
import uk.co.compendiumdev.restmud.engine.game.verbs.handlers.VerbCloseHandler;
...
public class VerbCloseHandlerTest {
    ...
    class VerbHandlerGame implements GameGenerator {
        ...
        public void generateInto(MudGame game) {
            ...
            defn.addGate("onewayegate", "1", "e", "2", GateDirection.ONE_WAY, GateStatus.OPEN).gateIsHidden(false);
            defn.addGate("onewaysouthgate", "1", "s", "2", GateDirection.ONE_WAY, GateStatus.CLOSED).gateIsHidden(false);
    ...
    public void canCloseAnOpenGate(){
        ...
        final MudLocationDirectionGate gate = game.getGateManager().getGateNamed("onewayegate");

        Assert.assertTrue("Expected Gate Open based on defn", gate.isOpen());

        VerbCloseHandler closeHandler = (VerbCloseHandler) new VerbCloseHandler().setGame(game);
    ...
    public void cannotCloseAClosedGate(){
        ...
        final MudLocationDirectionGate gate = game.getGateManager().getGateNamed("onewaysouthgate");
